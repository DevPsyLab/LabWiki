...
```{r}
round(data.frame(psych::describe(df)), 2)
```
...
```{r}
round(colMeans(df, na.rm = TRUE), 2)
round(apply(df, 2, function(x) mean(x, na.rm = TRUE)), 2)
...
```{r}
round(apply(df, 2, function(x) median(x, na.rm = TRUE)), 2)
...
```{r}
round(apply(df, 2, function(x) Mode(x, multipleModes = "mean")), 2)
