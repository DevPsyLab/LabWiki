...
This includes, but is not limited to, [pre- or co-registering](#sec-preregistration) studies, [sharing analysis scripts and data](#sec-sharing), using [version control](#sec-versionControl) (GitLab), submitting [preprints](https://psyarxiv.com) when submitting a manuscript to a journal, and providing support for other labs' attempts to replicate and reproduce our findings.
Our lab's template for projects on the [Open Science Framework](https://osf.io) (OSF) is located here[^1]: <https://osf.io/4w9sv>.

# Pre-Registration (or Co-Registration) {#sec-preregistration}
...
# Sharing Data, Analysis Scripts, and Research Materials {#sec-sharing}
...
#### Variable Types {#sec-variableTypes}
...
#### Measurement Units {#sec-measurementUnits}
...
The [Open Science Framework](https://osf.io) (OSF) is a website for hosting pre-registrations—and formerly data, analysis code, research-related materials, and pre-prints—to improve replicability and reproducibility of findings.
...
### Create Anonymous View-Only Links for Anonymous Peer Review {#sec-anonymousViewOnlyLink}
...
GitHub is a website for hosting and sharing pre-registrations, data, analysis code, research-related materials, and pre-prints to improve replicability and reproducibility of findings.
For each paper project, create a new repository on GitHub and add the relevant contributors, including Dr. Petersen.
Our lab's template for projects on GitHub can be found here[^1]: <https://research-git.uiowa.edu/PetersenLab/Template>.

### Pre-registration {#sec-osfPreregistration}

### Data {#sec-osfData}

When sharing data, make sure to share only <u>***deidentified***</u> data.
For any variables that have participant-identifying information, make sure to remove them from the data object before sharing.
...
### Data Dictionary {#sec-osfDataDictionary}

For each paper project, we export a `.csv` file with the subset of the [Data Dictionary](#sec-dataDictionary) variables used for that specific paper.
We upload that `.csv` file to the OSF.
The formatting of the Data Dictionary is described [here](#sec-dataDictionary).

### Analysis Code {#sec-osfAnalysisCode}

### Research Materials {#sec-osfMaterials}

### Preprint {#sec-osfPreprint}

## Zenodo {#sec-zenodo}

For instructions how to share and preserve research materials and code via GitHub with a DOI, see here: <https://www.lib.uiowa.edu/data/share-and-preserve-your-code/> (archived at: <https://perma.cc/ND4B-W2JF>).

# Manuscript Submission to a Journal {#sec-journal}

Before submitting a manuscript to a journal, make sure to post the relevant materials on the [OSF](#sec-osf), as described [here](#sec-osf), and post the preprint, as described [here](#sec-preprint).
When preparing a manuscript for submission to a journal, make sure to follow the Author Guidelines for each journal.
After finalizing the manuscript in accordance with journal guidelines and when you are ready to submit the paper to the journal (but before submission), post the [preprint](#sec-preprint) on [PsyArXiv](https://psyarxiv.com).
...
In the manuscript submission, create and use [anonymous view-only OSF links](#sec-anonymousViewOnlyLink) (for blind review).
In the preprint submission, use the general OSF links (not the [anonymous view-only OSF links](#sec-anonymousViewOnlyLink)) that will become viewable when the manuscript is accepted for publication (i.e., when you make the OSF repo public).

# When the Manuscript is Accepted for Publication {#sec-accepted}
...
1. Make the GitHub repo public and [create a DOI link](https://www.lib.uiowa.edu/data/share-and-preserve-your-code/) (archived at: <https://perma.cc/ND4B-W2JF>) for the repository that can be used for citing it
1. Submit the finalized, unblinded manuscript (and any tables, figures, and the supplement; with the public GitHub link; removing any highlighting or tracked changes) to the NIHMS system: <https://www.nihms.nih.gov/submission/create/>
    - After you submit the manuscript to NIHMS, send Dr. Petersen the NIHMS ID for the submission.
    We are required to report published papers to funding agencies.
1. Make sure the finalized, unblinded manuscript (and any tables, figures, and the supplement; with the public GitHub link; removing any highlighting or tracked changes) is uploaded as an updated version of the [preprint](#sec-preprint) on [PsyArXiv](https://psyarxiv.com).

# Adapting Open Science to Longitudinal Research {#sec-longitudinal}
