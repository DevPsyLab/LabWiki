...
## To begin
  ...
  3. Create Personal Access Token (https://docs.gitlab.com/ee/user/profile/personal_access_tokens.html or https://research-git.uiowa.edu/profile/personal_access_tokens) to access GitLab repositories from GitHub Desktop.
      -  For application name, put "GitHub"
      -  For expiration date, leave it blank
      -  For scopes, select everything
  4. When cloning a repository from GitLab using GitHub desktop, GitHub Desktop will ask for your username and password. Enter your HawkID as your username and your Personal Access Token as your password.
...
1. Follow the folder structure of the Petersen Lab template (https://research-git.uiowa.edu/petersenlab/Template)
...
1. `git clone https://research-git.uiowa.edu/petersenlab/srs/SRS-DataProcessing.git .`
1. `git remote set-url --add origin https://research-git.uiowa.edu/petersenlab/srs/SRS-DataProcessing.git`
1. `git remote -v`
...
1. Copy the large files into the repo.
1. In GitHub Desktop, commit and push the large file to the cloud version of the repo.
1. (Or, if not using GitHub Desktop, can commit in the command prompt (https://docs.gitlab.com/ee/topics/git/lfs/):
    ```
...
1. `git add -A`
1. `git status`
1. `git commit -m "Message"` (where "Message" is the summary message of the commit)
1. `git push`
1. `git status`
...
1. Add .gitignore file with .Rhistory
