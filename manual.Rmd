...
For general information regarding the scheduling expectations of the Developmental Psychopathology Lab, please refer to the section on [Scheduling Expectations](#schedulingExpectations).
...
1. Once the sheet has gone around the group once and everyone has had the opportunity to remove their name from days they cannot work, pass the sheet around, from lowest number to highest number, and every RA signs up for one day per turn.
...
1. On the "RAs" Board, there are categories of items that are performed in our lab
...
1. When the task is completed, and does not need to be followed up by another person, move the card from 'Doing' to 'Done'
...
| <!-- -->      | <!-- -->                         | <!-- -->                                                    | <!-- -->                                                                                                        |
|:--------------|:---------------------------------|:------------------------------------------------------------|:----------------------------------------------------------------------------------------------------------------|
| Members       | 01-Lab Reports                   | _Individual RA files, by HawkID_                            |                                                                                                                 |
|               | _Individual RA files, by HawkID_ |                                                             |                                                                                                                 |
| Miscellaneous | Professional Development         |                                                             |                                                                                                                 |
|               | RAs                              | RA Lab Hours                                                | _Folders are broken down by Semester and Year, and include RA Lab, Availability, and Weekend On-Call Schedules_ |
|               |                                  | RA Task Assignments                                         | _Folders are broken down by Semester and Year, and include Coding, EntryCheck, and Lab Cleaning Schedules_      |
...
|               | REDCap                           | Videos                                                      |                                                                                                                 |
|               |                                  | _PDF text versions of video training information, by topic_ |                                                                                                                 |
...
- Frame things in the negative (or the absence of negative)—i.e., do not provide reassurance ("It's going to be okay" or "It's not going to hurt.")
...
1. Setting Up Computer: 
    ...
    d. Checks:
        i. Check for good volume 
            1. As quiet as it can be (so parent and child cannot hear) while still being able to hear with good quality (**volume 28 or lower on computer**)
...
1. After Video Recording:
    ...
    c. **Copy** LV1 video from `C:` drive to the relevant folder on the lab drive.
        i. LV1 videos:
        `\\iowa.uiowa.edu\Shared\ResearchData\rdss_itpetersen\Lab\Studies\School_Readiness_Study\Videos\Lab Visit 1\Cohort #\T#\`
...
When saving videos for participants for whom we do not have a signed media release, append `_NMR` to the filename (e.g., `VideoName_NMR.mp4`).
...
**[XXX]NOTE: THIS SECTION (BELOW) SEEMS STUDY-SPECIFIC. CAN WE MOVE IT OUT OF THE GENERAL LAB MANUAL?**
...
1. To create videos:
    ...
    e. Once the accurate clip time window is selected, go to File &rarr; Save, and save the clip to the computer desktop with a new name (e.g., clip1, clip2, etc., so that you do not overwrite the new clips you create) 
...
**[XXX]NOTE: <ins>PARTS</ins> OF THIS SECTION SEEM STUDY-SPECIFIC. CAN WE MOVE IT OUT OF THE GENERAL LAB MANUAL?**
...
- To preview the wiki before you commit changes to the site, you can create the `.html` files locally (in a folder called `_site`) by running the following command:
